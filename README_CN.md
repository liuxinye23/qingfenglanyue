...
- 📱 **机器人**：支持钉钉、飞书长连接，在手机端与 INNP 对话（配置与命令详见 [机器人使用说明](docs/robot.md)）
...
- **Burp Suite 插件**：`plugins/burp-suite/innp-burp-extension/`  
  构建产物：`plugins/burp-suite/innp-burp-extension/dist/innp-burp-extension.jar`  
  说明文档：`plugins/burp-suite/innp-burp-extension/README.zh-CN.md`
...
- **可选 MCP 服务**：项目中的 [`mcp-servers/`](mcp-servers/README_CN.md) 目录提供独立 MCP（如反向 Shell），采用标准 MCP stdio，可在 INNP（设置 → 外部 MCP）、Cursor、VS Code 等任意支持 MCP 的客户端中使用。
...
**快速开始（使用预构建知识库）：**
1. **下载知识数据库**：从 [GitHub Releases](https://github.com/liuxinye23/qingfenglanyue/releases) 下载预构建的知识数据库文件。
2. **解压并放置**：将下载的知识数据库文件（`knowledge.db`）解压后放到项目的 `data/` 目录下。
3. **重启服务**：重启 INNP 服务，知识库即可直接使用，无需重新构建索引。
...
- [机器人使用说明（钉钉 / 飞书）](docs/robot.md)：在手机端通过钉钉、飞书与 INNP 对话的完整配置步骤、命令与排查说明，**建议按该文档操作以避免走弯路**。
...
INNP 采用 **Apache License 2.0** 开源许可。  
...
INNP 是一个专业的安全测试平台，旨在帮助安全研究人员、渗透测试人员和IT专业人员在**获得明确授权**的情况下进行安全评估和漏洞研究。
