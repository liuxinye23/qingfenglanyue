...
func TestExecutor_ExecuteTool_MissingBinaryReturnsFriendlyError(t *testing.T) {
	executor, _ := setupTestExecutor(t)
	executor.config.Tools = []config.ToolConfig{
		{
			Name:    "missing-binary",
			Command: "definitely-not-installed-binary-for-test",
			Enabled: true,
			Args:    []string{"--version"},
		},
	}
	...
	res, err := executor.ExecuteTool(context.Background(), "missing-binary", map[string]interface{}{})
	if err != nil {
		t.Fatalf("ExecuteTool returned unexpected error: %v", err)
	}
	if res == nil || !res.IsError {
		t.Fatalf("expected tool result error, got %#v", res)
	}
	if len(res.Content) == 0 || !strings.Contains(res.Content[0].Text, "configured tool paths or PATH") {
		t.Fatalf("unexpected response: %#v", res)
	}
}

func TestExecutor_GetConfiguredToolsHealth(t *testing.T) {
	...
	executor.config.Tools = []config.ToolConfig{
		...
		{
			Name:    "missing-binary",
			Command: "definitely-not-installed-binary-for-health-test",
			Enabled: true,
		},
		{
			Name:    "disabled-tool",
	...
	items := executor.GetConfiguredToolsHealth()
	if len(items) != 3 {
		t.Fatalf("expected 3 health items, got %d", len(items))
	}

	byName := map[string]ToolHealth{}
	for _, item := range items {
		byName[item.Name] = item
	}

	if byName["registered-internal"].Status != "ok" || !byName["registered-internal"].Registered {
		t.Fatalf("unexpected registered-internal health: %#v", byName["registered-internal"])
	}
	if byName["missing-binary"].Status != "missing_binary" {
		t.Fatalf("unexpected missing-binary health: %#v", byName["missing-binary"])
	}
	if !strings.Contains(byName["missing-binary"].Reason, "configured tool paths or PATH") {
		t.Fatalf("expected PATH reason, got %#v", byName["missing-binary"])
	}
	if byName["disabled-tool"].Status != "disabled" {
		t.Fatalf("unexpected disabled-tool health: %#v", byName["disabled-tool"])
	}
}
