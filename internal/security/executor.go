...
// GetConfigWarnings returns configuration-time warnings relevant to tool loading.
func (e *Executor) GetConfigWarnings() []string {
	if e == nil || e.config == nil {
		return nil
	}

	val := reflect.ValueOf(*e.config)
	field := val.FieldByName("ToolConfigWarnings")
	if !field.IsValid() || field.Kind() != reflect.Slice {
		return nil
	}

	warnings, ok := field.Interface().([]string)
...
// ToolHealth describes runtime readiness for a configured tool.
...
type toolExecutionPreflight struct {
	commandPath    string
	missingParams  []string
	missingCommand bool
}

// GetConfiguredToolsHealth returns health information for configured tools only.
func (e *Executor) GetConfiguredToolsHealth() []ToolHealth {
	health := make([]ToolHealth, 0, len(e.config.Tools))
	registered := map[string]bool{}

	if e.mcpServer != nil {
		for _, tool := range e.mcpServer.GetAllTools() {
			registered[tool.Name] = true
		}
	}

	for i := range e.config.Tools {
		toolCfg := &e.config.Tools[i]
		item := ToolHealth{
			Name:           toolCfg.Name,
			Enabled:        toolCfg.Enabled,
			Registered:     registered[toolCfg.Name],
			Command:        toolCfg.Command,
			IsInternal:     strings.HasPrefix(toolCfg.Command, "internal:"),
			RequiredParams: requiredToolParams(toolCfg),
		}
		if len(toolCfg.Parameters) > 0 {
			item.InputSchema = e.buildInputSchema(toolCfg)
		}

		switch {
		case !toolCfg.Enabled:
			item.Status = "disabled"
			item.StatusClass = "disabled"
			item.Reason = "tool is disabled in configuration"
		case item.IsInternal:
			item.Status = "ok"
			item.StatusClass = "ok"
			item.Reason = "internal tool handler is available"
		default:
			commandPath, err := e.resolveToolCommandPath(toolCfg.Command)
			if err != nil {
				item.Status = "missing_binary"
				item.StatusClass = "missing_binary"
				item.Reason = err.Error()
			} else {
				item.CommandPath = commandPath
				item.Status = "ok"
				item.StatusClass = "ok"
				item.Reason = "binary resolved in configured tool paths or PATH"
			}
		}

		if toolCfg.Enabled && !item.Registered {
			item.Warnings = append(item.Warnings, "tool is enabled in configuration but not registered in MCP")
			if item.Status == "ok" {
				item.Status = "unregistered"
				item.StatusClass = "unregistered"
				if item.Reason == "" {
					item.Reason = "tool is enabled in configuration but not registered in MCP"
				}
			}
		}
	...
	sort.Slice(health, func(i, j int) bool {
		return strings.ToLower(health[i].Name) < strings.ToLower(health[j].Name)
	})
...
func (e *Executor) ExecuteTool(ctx context.Context, toolName string, args map[string]interface{}) (*mcp.ToolResult, error) {
	...
	preflight := e.preflightToolExecution(toolConfig, args)
	if len(preflight.missingParams) > 0 {
		e.logger.Warn("missing required tool parameters",
			zap.String("toolName", toolName),
			zap.Strings("missingParams", preflight.missingParams),
		...
		return &mcp.ToolResult{
			Content: []mcp.Content{
				{
					Type: "text",
					Text: fmt.Sprintf("tool %s is missing required arguments: %s; received args: %v", toolName, strings.Join(preflight.missingParams, ", "), args),
				},
			},
			IsError: true,
		}, nil
	}
	if preflight.missingCommand {
		e.logger.Warn("tool command not available",
			zap.String("toolName", toolName),
			zap.String("command", toolConfig.Command),
		)
		return &mcp.ToolResult{
			Content: []mcp.Content{
				{
					Type: "text",
					Text: fmt.Sprintf("tool %s cannot run because command %q was not found in configured tool paths or PATH; install the binary on this host or add its directory to security.tool_path_dirs", toolName, toolConfig.Command),
				},
			},
			IsError: true,
		}, nil
	...
	// 验证命令参数
	if len(cmdArgs) == 0 && e.toolRequiresArguments(toolConfig) {
	...
	// 执行命令
	commandName := toolConfig.Command
	if preflight.commandPath != "" {
		commandName = preflight.commandPath
	}
	...
	if cb, ok := ctx.Value(ToolOutputCallbackCtxKey).(ToolOutputCallback); ok && cb != nil {
		...
		if err != nil && shouldRetryWithPTY(output) {
			...
			cmd2 := exec.CommandContext(ctx, commandName, cmdArgs...)
...
func (e *Executor) preflightToolExecution(toolConfig *config.ToolConfig, args map[string]interface{}) toolExecutionPreflight {
	result := toolExecutionPreflight{
		missingParams: missingRequiredParams(toolConfig, args),
	}
	...
	commandPath, err := e.resolveToolCommandPath(toolConfig.Command)
	if err != nil {
		result.missingCommand = true
		return result
	}
...
func (e *Executor) toolRequiresArguments(toolConfig *config.ToolConfig) bool {
	if toolConfig == nil {
		return false
	}
	return len(requiredToolParams(toolConfig)) > 0
}

func missingRequiredParams(toolConfig *config.ToolConfig, args map[string]interface{}) []string {
	if toolConfig == nil {
		return nil
	}
	...
	for _, param := range toolConfig.Parameters {
		...
		if !ok || value == nil || isEmptyToolArg(value) {
...
func requiredToolParams(toolConfig *config.ToolConfig) []string {
	if toolConfig == nil {
		return nil
	}
...
func isEmptyToolArg(value interface{}) bool {
	switch v := value.(type) {
	case string:
...
func (e *Executor) resolveToolCommandPath(command string) (string, error) {
	if e == nil || e.config == nil {
		return resolveToolCommandPath(command)
	}
	return resolveToolCommandPathWithDirs(command, e.config.ToolPathDirs)
}

func resolveToolCommandPath(command string) (string, error) {
	return resolveToolCommandPathWithDirs(command, nil)
}

func resolveToolCommandPathWithDirs(command string, extraDirs []string) (string, error) {
	command = strings.TrimSpace(command)
	if command == "" {
	...
	if commandLooksLikePath(command) {
		path, err := lookupCommandCandidate(command)
		if err != nil {
			return "", fmt.Errorf("command %q is not accessible: %w", command, err)
		}
		return path, nil
	}
	...
	for _, dir := range extraDirs {
		...
		baseCandidate := filepath.Join(dir, command)
		for _, candidate := range commandPathCandidates(baseCandidate) {
			path, err := lookupCommandCandidate(candidate)
			if err == nil {
				return path, nil
			}
		}
	...
	if err != nil {
		return "", fmt.Errorf("command %q was not found in configured tool paths or PATH", command)
	}
	return path, nil
}

func commandLooksLikePath(command string) bool {
	return filepath.IsAbs(command) || strings.ContainsAny(command, `/\`)
}

func lookupCommandCandidate(candidate string) (string, error) {
	return exec.LookPath(candidate)
}

func commandPathCandidates(base string) []string {
