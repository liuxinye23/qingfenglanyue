...
func RunDeepAgent(
	...
	reasoning.ApplyToEinoToolCallingChatModelConfig(baseModelCfg, &appCfg.OpenAI, reasoningClient)
	...
	if orchMode != "plan_execute" {
		...
		for _, sub := range effectiveSubs {
			...
			if instr == "" {
				instr = "你是 INNP 中的专业子代理，在授权渗透测试场景下协助完成用户委托的子任务。优先使用可用工具获取证据，回答简洁专业。"
			}
	...
	orchInstruction = injectSkillShortlistInstruction(ctx, orchInstruction, skillsRoot, userMessage, roleTools, mainTools, logger)
	if logger != nil {
