...
type SecurityConfig struct {
	...
	ToolPathDirs        []string     `yaml:"tool_path_dirs,omitempty" json:"tool_path_dirs,omitempty"`
...
func Load(path string) (*Config, error) {
	...
	if len(cfg.Security.ToolPathDirs) == 0 {
		cfg.Security.ToolPathDirs = []string{filepath.Join(configDir, ".tools", "bin")}
	} else {
		resolvedToolPathDirs := make([]string, 0, len(cfg.Security.ToolPathDirs))
		for _, dir := range cfg.Security.ToolPathDirs {
			...
			resolvedToolPathDirs = append(resolvedToolPathDirs, filepath.Clean(dir))
		}
		cfg.Security.ToolPathDirs = resolvedToolPathDirs
	}
