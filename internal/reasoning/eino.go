...
var reasoningExtraFieldKeys = map[string]struct{}{
	"thinking":         {},
	"reasoning":        {},
	"reasoning_effort": {},
	"output_config":    {},
...
// ApplyToEinoToolCallingChatModelConfig applies reasoning settings for tool-calling
// chat models. DeepSeek reasoning endpoints reject tool_choice, so for DeepSeek
// compatibility we preserve only non-reasoning extra fields and suppress automatic
// reasoning-related request keys.
func ApplyToEinoToolCallingChatModelConfig(cfg *einoopenai.ChatModelConfig, oa *config.OpenAIConfig, client *ClientIntent) {
	if cfg == nil || oa == nil {
		return
	}
	if resolveWireProfile(oa, &oa.Reasoning) != wireDeepseek {
		ApplyToEinoChatModelConfig(cfg, oa, client)
		return
	}
	mergeNonReasoningExtraFields(cfg, oa.Reasoning.ExtraRequestFields)
}
...
func mergeNonReasoningExtraFields(cfg *einoopenai.ChatModelConfig, extras map[string]any) {
	if cfg == nil || len(extras) == 0 {
		return
	}
	for k, v := range extras {
		if _, blocked := reasoningExtraFieldKeys[strings.ToLower(strings.TrimSpace(k))]; blocked {
			continue
		}
		if cfg.ExtraFields == nil {
			cfg.ExtraFields = make(map[string]any)
