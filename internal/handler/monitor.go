...
type DirectMCPToolHealth struct {
...
type ExternalMCPServerHealth struct {
	Name      string `json:"name"`
...
type ToolsHealthResponse struct {
	Configured    []security.ToolHealth     `json:"configured"`
	DirectMCP     []DirectMCPToolHealth     `json:"direct_mcp"`
	ExternalMCP   []ExternalMCPServerHealth `json:"external_mcp"`
	Summary       map[string]int            `json:"summary"`
	StatusBuckets map[string]int            `json:"status_buckets,omitempty"`
...
// GetToolsHealth returns current readiness for configured tools, direct MCP tools, and external MCP servers.
func (h *MonitorHandler) GetToolsHealth(c *gin.Context) {
	configured := []security.ToolHealth{}
	if h.executor != nil {
		configured = h.executor.GetConfiguredToolsHealth()
	}

	configuredNames := make(map[string]struct{}, len(configured))
	for _, tool := range configured {
		configuredNames[tool.Name] = struct{}{}
	}

	direct := make([]DirectMCPToolHealth, 0)
	if h.mcpServer != nil {
		for _, tool := range h.mcpServer.GetAllTools() {
			if _, exists := configuredNames[tool.Name]; exists {
				continue
			}
			direct = append(direct, DirectMCPToolHealth{
				Name:        tool.Name,
	...
	sort.Slice(direct, func(i, j int) bool {
		return strings.ToLower(direct[i].Name) < strings.ToLower(direct[j].Name)
	})

	external := make([]ExternalMCPServerHealth, 0)
	if h.externalMCPMgr != nil {
		configs := h.externalMCPMgr.GetConfigs()
		toolCounts := h.externalMCPMgr.GetToolCounts()
		for name, cfg := range configs {
			...
			errMsg := ""
			if client, exists := h.externalMCPMgr.GetClient(name); exists {
				status = client.GetStatus()
			}
			if status == "error" {
				errMsg = h.externalMCPMgr.GetError(name)
			}
			external = append(external, ExternalMCPServerHealth{
				Name:      name,
				Enabled:   cfg.ExternalMCPEnable,
				Status:    status,
	...
	if h.executor != nil {
		warnings = append(warnings, h.executor.GetConfigWarnings()...)
	}
	for _, tool := range configured {
		bucket := strings.TrimSpace(tool.StatusClass)
		if bucket == "" {
			bucket = strings.TrimSpace(tool.Status)
		}
		if bucket == "" {
			bucket = "unknown"
		}
		...
		if tool.Status == "ok" {
			summary["configured_ok"]++
		} else {
			summary["configured_issues"]++
		}
	}
	for _, server := range external {
		if server.Status == "connected" || server.Status == "ok" {
			summary["external_ok"]++
		}
	}

	capabilities := map[string]any{
		"configured_tools_ready_ratio": map[string]int{
			"ok":    summary["configured_ok"],
			"total": summary["configured_total"],
		},
	}

	c.JSON(http.StatusOK, ToolsHealthResponse{
