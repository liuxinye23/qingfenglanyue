...
func (h *SkillsHandler) GetSkills(c *gin.Context) {
	...
	for _, s := range allSummaries {
		skillInfo := map[string]interface{}{
			...
			"domains":           s.Domains,
			"stages":            s.Stages,
			"target_types":      s.TargetTypes,
			"bundle_of":         s.BundleOf,
			"depends_on":        s.DependsOn,
			"is_bundle":         s.IsBundle,
			"recommended_tools": s.RecommendedTools,
			"required_tools":    s.RequiredTools,
			"role_hints":        s.RoleHints,
			"autoload_priority": s.AutoLoadPriority,
	...
	if searchKeyword != "" {
		...
		for _, skillInfo := range allSkillsInfo {
			...
			aliasJoined := ""
			if tr, ok := skillInfo["aliases"].([]string); ok {
				aliasJoined = strings.ToLower(strings.Join(tr, " "))
			}
			domainJoined := ""
			if tr, ok := skillInfo["domains"].([]string); ok {
				domainJoined = strings.ToLower(strings.Join(tr, " "))
			}
			stageJoined := ""
			if tr, ok := skillInfo["stages"].([]string); ok {
				stageJoined = strings.ToLower(strings.Join(tr, " "))
			}
			targetJoined := ""
			if tr, ok := skillInfo["target_types"].([]string); ok {
				targetJoined = strings.ToLower(strings.Join(tr, " "))
			}
			bundleJoined := ""
			if tr, ok := skillInfo["bundle_of"].([]string); ok {
				bundleJoined = strings.ToLower(strings.Join(tr, " "))
			}
			dependsJoined := ""
			if tr, ok := skillInfo["depends_on"].([]string); ok {
				dependsJoined = strings.ToLower(strings.Join(tr, " "))
			}
			roleJoined := ""
			if tr, ok := skillInfo["role_hints"].([]string); ok {
				roleJoined = strings.ToLower(strings.Join(tr, " "))
			}
			recoToolsJoined := ""
			if tr, ok := skillInfo["recommended_tools"].([]string); ok {
				recoToolsJoined = strings.ToLower(strings.Join(tr, " "))
			}
			reqToolsJoined := ""
			if tr, ok := skillInfo["required_tools"].([]string); ok {
				reqToolsJoined = strings.ToLower(strings.Join(tr, " "))
			}
			if strings.Contains(id, keywordLower) ||
				...
				strings.Contains(aliasJoined, keywordLower) ||
				strings.Contains(domainJoined, keywordLower) ||
				strings.Contains(stageJoined, keywordLower) ||
				strings.Contains(targetJoined, keywordLower) ||
				strings.Contains(bundleJoined, keywordLower) ||
				strings.Contains(dependsJoined, keywordLower) ||
				strings.Contains(roleJoined, keywordLower) ||
				strings.Contains(recoToolsJoined, keywordLower) ||
				strings.Contains(reqToolsJoined, keywordLower) {
...
// GetSkillRecommendations returns ranked skills for the current request intent.
func (h *SkillsHandler) GetSkillRecommendations(c *gin.Context) {
	query := strings.TrimSpace(c.Query("query"))
	if query == "" {
		query = strings.TrimSpace(c.Query("search"))
	}
	role := strings.TrimSpace(c.Query("role"))
	toolsRaw := strings.TrimSpace(c.Query("tools"))
	var availableTools []string
	if toolsRaw != "" {
		for _, item := range strings.Split(toolsRaw, ",") {
			item = strings.TrimSpace(item)
			if item != "" {
				availableTools = append(availableTools, item)
			}
		}
	}
	limit := 5
	...
	result, err := skillpackage.RecommendSkillsDetailed(h.skillsRootAbs(), skillpackage.SkillRecommendOptions{
		Query:          query,
		Role:           role,
	...
	if err != nil {
		h.logger.Error("skill 推荐失败", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{
		...
		"inferred_targets": result.InferredTargets,
		"limit":            limit,
	})
}
...
func (h *SkillsHandler) GetSkill(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"skill": map[string]interface{}{
			...
			"stages":            skill.Stages,
			"target_types":      skill.TargetTypes,
			"bundle_of":         skill.BundleOf,
			"depends_on":        skill.DependsOn,
			"is_bundle":         skill.IsBundle,
			"recommended_tools": skill.RecommendedTools,
			"required_tools":    skill.RequiredTools,
			"role_hints":        skill.RoleHints,
			"autoload_priority": skill.AutoLoadPriority,
			...
			"quality_warnings":  skill.QualityWarnings,
...
func (h *SkillsHandler) GetSkillStats(c *gin.Context) {
	...
	for _, skillName := range skillList {
		summary, _ := skillpackage.LoadSkill(h.skillsRootAbs(), skillName, skillpackage.LoadOptions{Depth: "summary"})
		...
		statsList = append(statsList, map[string]interface{}{
			...
			"quality_score": func() int {
				if summary == nil {
					return 0
				}
				return summary.QualityScore
			}(),
			"quality_warnings": func() []string {
				if summary == nil {
					return nil
				}
				return summary.QualityWarnings
			}(),
		})
	}
