...
func (h *AgentHandler) prepareMultiAgentSession(req *ChatRequest) (*multiAgentPrepared, error) {
	...
	memoryContext := ""
	if mem, memErr := h.db.GetConversationMemory(conversationID); memErr == nil {
		memoryContext = buildConversationMemoryContext(mem)
	} else {
		h.logger.Warn("load conversation memory failed", zap.String("conversationId", conversationID), zap.Error(memErr))
	}
	...
	return &multiAgentPrepared{
		...
		EffectiveOrchestration: decideEffectiveOrchestration(req, finalMessage, roleTools),
	}, nil
}
